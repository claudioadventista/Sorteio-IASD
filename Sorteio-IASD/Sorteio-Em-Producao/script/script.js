...
somPalmas.volume = 0.3;somTambor.volume = 0.3;somPrato.volume = 0.3;player.volume = 0.3;let confere = "";
...
const LANG = {
    pt: {
        ...
        aleatorioPlace: "Ex: 123,12,474,8",
        ...
        creditoForm: " Crédito",
        // listas
        listaCadastrado: "Lista de Cadastrados:",
        ...
        but_sortear: "SORTEAR PRÓXIMO",
        but_historico: "HISTÓRICO",
        but_sortValido: "Sorteio válido",
        but_sortInvalido: "Sorteio inválido",
    ...
    en: {
        ...
        creditoForm: " Credit",
        //
        listaCadastrado: "Registered List:",
        listaParticipante: "Participants List:",
        ...
        but_sortear: "DRAW NEXT",
        but_historico: "HISTORY",
        but_sortValido: "Valid draw",
        but_sortInvalido: "Invalid draw",
    ...
    es: {
        ...
        aleatorioPlace: "Ej: 123,12,474,8",
        ...
        creditoForm: " Crédito",
        listaCadastrado: "Lista de Registrados:",
        listaParticipante: "Lista de Participantes:",
        ...
        but_sortear: "SORTEAR SIGUIENTE",
        but_historico: "HISTORIAL",
        but_sortValido: "Sorteo válido",
        but_sortInvalido: "Sorteio válido",
...
// idioma padrao e o ingles, se houver algum prblenma, sempre vai ser o ingles
let idiomaAtual = 'en';

// Tenta pegar o idioma atual no local storage
try{
    const langSalva = localStorage.getItem('sistema_lang');
    if(langSalva === 'pt' || langSalva === 'en' || langSalva === 'es'){
        idiomaAtual = langSalva;
    }else{
        // Se não conseguir, atribui o ingles ao idioma atual
        idiomaAtual = 'en';
        // Seta o ingles como idioma atual no local storage
        localStorage.setItem('sistema_lang', 'en');
    }
}catch (e){
    alert('localStorage bloqueado, usando EN como padrão:', e);
    idiomaAtual = 'en';
}
...
document.getElementById('seletorIdioma').addEventListener('change', (e) => {
    ...
    atualizarBotaoImportar();
    mudarIdioma();
    // O código abaixo é para mostrar a forma correta da posicao dos sorteados 
    // 1'o' em portugues e 1'st' em ingles, ao mudar o idioma
    renderizarLista();
    credito();
...
function tocarMusica(id){
    ...
    // Salva a musica que que foi escolhida, ela vem no proximo sorteio 
    localStorage.setItem('musicaSorteio', id);
...
function setVolume(valor){
    if (audioAtual){
        ...
        if(valo < 1){
            document.getElementById('valorVolumeMusica').innerHTML = "<span class='completaZero' >00</span>" + valo + "%";
        }else if(valo >=10 && valo < 100){
            document.getElementById('valorVolumeMusica').innerHTML = "<span class='completaZero' >0</span>" + valo + "%";
        }else{
            document.getElementById('valorVolumeMusica').textContent = valo + '%';
        }
    }
}
...
function salvarImagem() {
    ...
    const abrevFotos =  localStorage.getItem('nomeAbrev');
    const bairroFotos = localStorage.getItem('nomeBairro');
    const cidadeFotos = localStorage.getItem('nomeCidade');
       ...
       logInput();
        ...
        confirme.onclick = () => {
            ...
            confirme.onclick = () => {
                ...
                ctx.strokeRect(15, 15, larguraCanvas - 30, alturaTotal - 30);y += alturaMin; ctx.shadowBlur = 0;

                // Ministerio
                ctx.font = '38px Arial'; ctx.fillStyle = '#a5f7d7ff'; ctx.textAlign = 'center';
                quebraTexto(ctx, minFoto, centroX, y, maxWidthTexto, 40); y += 80 + espacamentoExtra ;

                // Bencao
                ctx.font = '28px Arial'; ctx.fillStyle = '#ffffff';
                quebraTexto(ctx, textFoto, centroX, y, maxWidthTexto, 30); y += infoBencao.alturaTotal + espacamentoExtra;

                // Titulo
                ctx.font = 'bold 55px Arial'; ctx.shadowColor = '#ff00ff'; ctx.shadowBlur = 5;
                ctx.fillText(`${ganhadorImagem}`, centroX, y + 5); y += alturaTitulo;

                // Trofeu
                ctx.font = '150px Arial'; ctx.shadowColor = '#ffd700'; ctx.shadowBlur = 25;
                ctx.fillText('🏆', centroX, y + 130); y += alturaTrofeu + 70 + espacamentoExtra;

                // Nome
                ctx.font = 'bold 80px Arial'; ctx.fillStyle = '#ffffff'; ctx.shadowColor = '#00ffff'; ctx.shadowBlur = 5;
                quebraTexto(ctx, ultimoGanhador, centroX, y, maxWidthTexto, 95); y += infoNome.alturaTotal + 20 ;

                // Ordem
                ctx.font = 'bold 45px Arial'; ctx.shadowBlur = 0;
                ctx.fillText(`${nomesSorteados.length}${sorteioImagem}`, centroX, y); y += alturaOrdem;

                // Data
                ctx.font = '35px Arial';
                ctx.fillText(dataImagem, centroX, y + 10); y += alturaData;
                ...
                if (file) {
                    ...
                    reader.onload = function(event) {
                        ...
                        logo.onload = function() {
                            ...
                            y += alturaLogo + 35 + espacamentoExtra;
                            desenharRodape();
                        }
                ...
                function desenharRodape() {
                    const nomeSorteio = abrevFotos + ' ' + bairroFotos + ' - ' + cidadeFotos;
                    ctx.font = '28px Arial'; ctx.fillStyle = 'rgba(255, 255, 255, 0.5)';
                    quebraTexto(ctx, nomeSorteio, centroX, y, maxWidthTexto, 38);
...
function renderizarEspera() {
    ...
    if(listaEspera.length > 0){
        ...
        // Quando acaba todos os nomes da lista, entra aqui
        controleLista = 0;
        // Volta a lista de participantes
        listaEditavel.style.display = 'block';
        // Fecha a lista de espera
        document.getElementById('listaEspera').style.display = 'none';
        // Muda o nome da lista conforme o idioma atual
        if(idiomaAtual === 'en'){
            document.getElementById('tituloLista').innerText = 'Participants List:';
        }else{
            document.getElementById('tituloLista').innerText = 'Lista de Participantes:';
        }
        // some com os botoes espera e voltar, abaixo da lista
        ...
        // volta a mostrar a div com o campo pesquisa, o botao limpar pesquisa, e o botao importar todos
        document.getElementById('butSearch').style.visibility = 'visible';
        document.getElementById('butSearch').style.opacity = '1';
        // Atualiza o contador para mostrar a quantidade de nomes na lista de participantes
        atualizarContador();
    }
}
...
function sortearProximo() {
    ...
    botaoSecundario.style.visibility = 'hidden';
    // Inicia o sorteio
    sorteando = true;
    ...
    // aqui sorteia o ultimo nome ou numero
    if (disponiveis.length === 1) {
        const ganhador = disponiveis[0];
        nomeSorteadoOverlay.textContent = ganhador; 
        setTimeout(() => {
            // Se clicar no botao de confirmacao de sorteio valido no ultimo nome  'OK' entra aqui
             document.getElementById('butOk').onclick = () => {
                ...
                salvarDados();
                // Risca o sorteado da lista de participantes
                renderizarLista();
                atualizarContador();
                atualizarStatusOverlay();
                sorteando = false;
                btnProximo.style.display = 'inline-block';
                botaoSecundario.style.visibility = 'visible';
                document.getElementById('butOk').style.display = 'none';
                document.getElementById('butNone').style.display = 'none';
                btnProximo.disabled = false;
            };

            // Se clicar no botao de sorteio invalido entra aqui, 
            // reabilita os botões para o proximo sorteio
             document.getElementById('butNone').onclick = () => {
                disponiveis.length = 0;
                nomes.forEach(p => disponiveis.push(p));
                let indice = disponiveis.indexOf(ganhador);
                if(indice !== -1){
                    disponiveis.splice(indice, 1);
                }
                //removerDoSorteio(indice);
                moverParaEspera(indice);
           ...
           // localStorage.setItem('cte_historico', JSON.stringify(historico));
            ...
            setTimeout(() => {
                ...
                document.getElementById('butOk').style.display = 'inline-block';
                document.getElementById('butNone').style.display = 'inline-block';
            }, 5000);
        }, 500);
        return;
    }
    ...
    // Aqui sorteia se houver mais de um nome ou numero
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            localStorage.setItem('cte_historicoAtual', JSON.stringify(historicoAtual));
            nomeSorteadoOverlay.textContent = ganhador;
            // Destaca o ganhador com  um zoom na hora do sorteio
            nomeSorteadoOverlay.classList.add('ganhador');       
            soltarConfete();
            barraProgressoOverlay.classList.remove('mostrar');
            ...
            // Se o sorteio valeu, registrar o sorteado e segue poara o proximo sorteio
            document.getElementById('butOk').onclick = () => {
                nomesSorteados.push(ganhador);
                ultimoGanhador = ganhador;
                // ORDEM = tamanho do histórico + 1
            ...
            // Se o sorteio nao valeu, retira o nome para nao ser sorteado outra vez nessa rodada,
            // manda o nome para a lista de espera, reabilita os botões para o proximo sorteio
             document.getElementById('butNone').onclick = () => {
                // Zera alista de disponiveis
                disponiveis.length = 0;
                nomes.forEach(p => disponiveis.push(p));
                // Restaura a lista de disponiveis atualizada
                let indice = disponiveis.indexOf(ganhador);
                // Pega o indice do ganhador
                if(indice !== -1){
                    disponiveis.splice(indice, 1);
                }
                // Remove da lista de participantes, ou manda para a espera
                //removerDoSorteio(indice);
                moverParaEspera(indice);
...
// FUNCAO ESCONDE OS NOMES QUE ESTAO EM ESPERA
...
function limparDados() {
    ...
    confirme.onclick = () => {
        ...
        mudarIdioma();
        credito();
        mostrarErro('✅ Data erased', true);
        fechar();
    }
}

// FUNCAO que mostra a largura do monitor
function mostrarDimensoesMonitor() {
  const largura = window.screen.width;
  const altura = window.screen.height;
  const larguraUtil = window.screen.availWidth;
  const alturaUtil = window.screen.availHeight;
  const profundidadeCor = window.screen.colorDepth;
  const pixelRatio = window.devicePixelRatio;

  console.log(`Resolução total: ${largura} x ${altura}`);
  console.log(`Área útil (sem barra de tarefas): ${larguraUtil} x ${alturaUtil}`);
  console.log(`Profundidade de cor: ${profundidadeCor} bits`);
  console.log(`Densidade de pixels: ${pixelRatio}x`);

  // Mostra na página também
  const info = `
    Monitor: ${largura} x ${altura} px,  
    Área útil: ${larguraUtil} x ${alturaUtil} px<br>
    Cores: ${profundidadeCor} bits, 
    Pixel Ratio: ${pixelRatio}
  `;
  
  // Cria um div se não existir
  let div = document.getElementById('info-monitor');
  div.innerHTML = info;
}

mostrarDimensoesMonitor();
...
window.onload = function() {
    ...
    somTambor.muted = true;
    somPrato.muted = true;
    somPalmas.muted = true;
    

    // Busca pela a musica do ultimo sorteio, se nao encontrar, pega a primeira da lista
    let ultimaMusica = localStorage.getItem('musicaSorteio') || 1;
    
    // Comeca a tocar a primeira musica ao abrir o aplicativo e clicar em qualquer parte
    if (!jaTocou){
        player.play().catch(() => {
            const tocarNoClique = () => { 

                // toca a ultima musica encontrada
                tocarMusica(ultimaMusica);
